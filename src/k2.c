...
int verbose=0, wDOut=0; // вывод контрольных сообщений
...
//	while( (i=getopt(argc, argv, "t:s:v:") )!=-1) {
	while( (i=getopt(argc, argv, "wtsvc:") )!=-1)	{ // ключи вводить без значений, кроме "c"
		switch(i){
			case 't' :	TM=1; break;
			case 's' :	TS=1; break;
			case 'v' :	verbose=1; break;
			case 'w' :	wDOut=1; break;	// вывод D
 						...
 						case 6: if(buffer[i+2]==2)
								{
									...
									//				if (wDOut)	printf(" Pтек= %d ",buffer[i+4]);
										...
										case 0x73 : if (buffer[i+4]&0x04) {printf(" ЗС");p->to_MO3.to42.priem_K2=p->to_MO3.to41.PrM_K2=1;}else p->to_MO3.to41.PrM_K2=0;
													...
													if (wDOut)	if (buffer[i+4]&0x20)	printf(" ИДД");
													if (verbose)
													{
														printf(" ПРМ1 - ");
														if (buffer[i+4]&0x01) printf(" ПРС");
...
if (TS) p->toPR1[3]=0x8000;//8000-onn 0 dBm 0000-off TVK
													...
													if (wDOut) printf("\nНУС отв. TS= %d  PR1[3]= %Xh  toPR1[3]= %Xh\n",TS, p->PR1[3], p->toPR1[3]);
													break;
									}
								}
				...
				} // достаточное кол-во байт в буфере
			} // найден заголовок
...
//							p->toPR1[3]=0xFC00;	printf("\n----- ВКЛЮЧЕН ТВК С МАХ ОСЛАБЛЕНИЕМ -----\n");
							...
							if (wDOut) printf("\nТВК OFF. TS= %d  PR1[3]= %Xh  toPR1[3]= %Xh\n",TS, p->PR1[3], p->toPR1[3]);
						}
						N_COM++;break;
			   case 24:	N_COM++;break;
						//if(kbhit()) {N_COM++;getch();}break;
			   case 25:	write_com(10);
						printf(")  Команда НУС отправлена\n");
						if (wDOut) printf("\nНУС отпр. TS= %d  PR1[3]= %Xh  toPR1[3]= %Xh\n",TS, p->PR1[3], p->toPR1[3]);
						break;
			   ...
			   case 35: read_kvit();break;
 			   case 36: write_com24(24);
						...
						if (wDOut)		printf("  Начата посылка команды ДанИ.V.dV\n");

						comOK[24]=1;N_COM++;
						if (TS) p->toPR1[3]=0x8000;//8000-onn 0 dBm 0000-off TVK

						if (wDOut) printf("\nТВК ON if TS=1. TS= %d  PR1[3]= %Xh  toPR1[3]= %Xh\n",TS, p->PR1[3], p->toPR1[3]);
...
//----------------------------------------------------------------------------
			   case 37: if (TM) {N_COM++;break;}
						...
						{
							if (verbose) printf("			СТАРТ Tstart= %d  Tpr= %d\n",Tstart,Tpr);
							Tstart+=30;
...
//								cr_com42=p->from_MO3.from42.cr_com;
						...
						if ( ((num_com==13) || (num_com==14)) && p->to_MO3.to41.sost_CC_K2)
						{
							...
							if (wDOut) printf("\nК2 ok. TS= %d  PR1[3]= %Xh  toPR1[3]= %Xh\n",TS, p->PR1[3], p->toPR1[3]);
						}
						else
						{
